...
public class OWalletTransaction {
    ...
    public String transactionEth(ECKeyPair ecKeyPair, String ethAmount, String toAddress, String gas_price, String gas_limit, String data) throws IOException, InterruptedException {
        ...
        nonce = this.getWalletAddressNonce(nonce, walletAddress);
    ...
    private BigInteger getWalletAddressNonce(BigInteger nonce, String walletAddress) {
        WalletGroupDetailExample groupDetailExample = new WalletGroupDetailExample();
        groupDetailExample.or().andWalletAddressEqualTo(walletAddress);
        List<WalletGroupDetail> walletGroupDetails = walletGroupDetailMapper.selectByExample(groupDetailExample);
        if (walletGroupDetails != null && walletGroupDetails.size() > 0) {
            WalletGroupDetail walletGroupDetail = walletGroupDetails.get(0);
            if (nonce.equals(walletGroupDetail.getNonce())) {
                nonce = nonce.add(new BigInteger("1"));
            }
            WalletGroupDetail updateWallet = new WalletGroupDetail();
            updateWallet.setId(walletGroupDetail.getId());
            updateWallet.setNonce(nonce);
            walletGroupDetailMapper.updateByPrimaryKeySelective(updateWallet);
        }
    ...
    public String transactionOnContract(ECKeyPair ecKeyPair, String OCNAmount, String toAddress, String gas_price, String gas_limit, String data, String ERC20Address) throws IOException, InterruptedException {
        ...
        if (txHashResponse.result == null || "".equals(txHashResponse.result)) {
            logger.warn("ETHSCAN  transfer  error， \n " +
                    "getnonce url: " + getNonceForAddress(walletAddress) + "\n" +
                    "getnonce responseResult: " + responseResult + "\n" +
                    "nonce: " + nonce + "\n" +
                    "transaction url:" + url + "\n" +
    ...
    public static List<TransactionsResponse.CustomTransaction> getTransactionList(String address, String startBlockNumber, String endBlockNumber) throws Exception {
        ...
        try {
            ...
            logger.error(String.format("scan block faild,url:{%s},responseResult:{%s}", url, responseResult), e);
            throw new Exception("scan block faild");
        }
    ...
    public String doubleTransactionETH(Web3j web3j, String address, ECKeyPair ecKeyPair, String ETHAmount, String gas_price, String gas_limit, String data) {
        try {
            ...
            logger.error(e.getMessage(), "node transfer ETH faild："+e);
            try {
                return transactionEth(ecKeyPair, ETHAmount, address, gas_price, gas_limit, data);
            } catch (Exception e1) {
                logger.error(e.getMessage(),"ETHSCAN transfer ETH faild："+ e);
                return Constants.TRANSFER_ERROR;
            }
        }
    ...
    public static TransactionVerificationInfo verifyTransaction(String txHash) {
        try {
            ...
            if (recentBlockNumber == null || transactionReceiptResponse == null || transactionReceiptResponse.getResult() == null || transactionReceiptResponse.getResult().getBlockNumber() == null) {
                logger.info("txhash get transtion is null：txhash=", txHash);
                return null;
            }
